...
def harrison_ford
  ...
  Movie.select(:id,:title).joins(:actors).where("ORD <> 1 AND name = 'Harrison Ford'")
end
...
def biggest_cast
  ...
  Movie.select(:id,:title).joins(:castings).group(:id).order('COUNT(*) DESC').limit(3)
    ...
    # Movie.select(:id,:title).joins(:castings).group("castings.movie_id").order('COUNT(*) DESC').limit(3)
end
